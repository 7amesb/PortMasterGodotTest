...
echo "lowspectest44.sh running" >> roms/7ames-log.txt

# PortMaster preamble
XDG_DATA_HOME=${XDG_DATA_HOME:-$HOME/.local/share}

echo "XDG_DATA_HOME = $XDG_DATA_HOME" >> roms/7ames-log.txt

if [ -d "roms/tools/PortMaster/" ]; then
	controlfolder="roms/tools/PortMaster/"
elif [ -d "EASYROMS/tools/PortMaster/" ]; then
	controlfolder="EASYROMS/tools/PortMaster/"
elif [ -d "$XDG_DATA_HOME/tools/PortMaster/" ]; then
	controlfolder="$XDG_DATA_HOME/tools/PortMaster/"
...
echo "controlfolder = $controlfolder" >> roms/7ames-log.txt

source $controlfolder/control.txt
[ -f "${controlfolder}/mod_${CFW_NAME}.txt" ] && source "${controlfolder}/mod_${CFW_NAME}.txt"
get_controls

echo "Device Arch = $DEVICE_ARCH" >> roms/7ames-log.txt
echo "directory = $directory" >> roms/7ames-log.txt

# Adjust these to your paths and desired godot version
GAMEDIR=/$directory/ports/lowspectest44
godot_runtime="godot_4.4"
godot_executable="godot44.$DEVICE_ARCH"
pck_filename="lowspectest44.pck"
gptk_filename="lowspectest44.gptk"
